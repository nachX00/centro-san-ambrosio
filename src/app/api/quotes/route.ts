import { NextResponse } from "next/server";
import { isValidRut } from "@/lib/rut";

export async function POST(request: Request) {
  let body: unknown;
  try {
    body = await request.json();
  } catch {
    return NextResponse.json({ error: "JSON inválido" }, { status: 400 });
  }

  const data = body as {
    name?: string;
    rut?: string;
    phone?: string;
    email?: string;
    prevision?: string;
    items?: unknown[];
    consent?: boolean;
  };

  if (!data.name?.trim() || !data.phone?.trim() || !data.consent) {
    return NextResponse.json({ error: "Faltan campos obligatorios" }, { status: 400 });
  }
  if (data.rut && !isValidRut(data.rut)) {
    return NextResponse.json({ error: "RUT inválido" }, { status: 400 });
  }
  if (!Array.isArray(data.items) || data.items.length === 0) {
    return NextResponse.json({ error: "Sin exámenes" }, { status: 400 });
  }

  const id = crypto.randomUUID();
  const supabaseUrl = process.env.SUPABASE_URL;
  const supabaseKey = process.env.SUPABASE_SERVICE_ROLE;

  if (supabaseUrl && supabaseKey) {
    const insert = await fetch(`${supabaseUrl}/rest/v1/quotes`, {
      method: "POST",
      headers: {
        apikey: supabaseKey,
        Authorization: `Bearer ${supabaseKey}`,
        "Content-Type": "application/json",
        Prefer: "return=minimal",
      },
      body: JSON.stringify({
        id,
        name: data.name,
        rut: data.rut ?? null,
        phone: data.phone,
        email: data.email ?? null,
        prevision: data.prevision ?? null,
        items: data.items,
        consent: true,
      }),
    });
    if (!insert.ok) {
      return NextResponse.json({ error: "No se pudo guardar" }, { status: 502 });
    }
  }

  return NextResponse.json({ id, stored: Boolean(supabaseUrl && supabaseKey) });
}
